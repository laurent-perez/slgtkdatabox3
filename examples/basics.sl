...
define slsh_main ()
{   
   variable box, databox, graph, win, x, y, vbox, txt;
   ...
   txt = gtk_label_new ("Draw a selection with the left button pressed, then click into the selection.\nUse the right mouse button to zoom out.\nShift + right mouse button zooms to default.");
   gtk_box_pack_start (vbox, txt, FALSE, FALSE, 0);
   databox = gtk_databox_new ();
   graph = gtk_databox_points_new (x, y, gdk_red, 5);
   gtk_databox_graph_add (databox, graph);
   graph = gtk_databox_bars_new (x, y, gdk_blue, 3);
   gtk_databox_graph_add (databox, graph);   
   graph = gtk_databox_lines_new (x, abs (y), gdk_green, 0);
   % graph = gtk_databox_lines_new (x, abs (y));
   gtk_databox_graph_add (databox, graph);
